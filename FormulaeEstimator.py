...
class FormulaeEstimator(object):
    def __init__(self, z3processor, conjunction=False, classes=[]):
        ...
        self.groups_formula = []            # 存合并后每个Group的公式 shape[2,n_features] 第一行上界；第二行下界
        self.visited = []
        self.sat_group = []
                    ...
                    for feature in range(self._n_features):
                        if o_visited[0][feature] == 1:
                            if visited[0][feature] == 0:
                                ...
                                formula[0][feature] = min(formula[0][feature], o_formula[0][feature])       # 取交集
                        if o_visited[1][feature] == 1:
                            if visited[1][feature] == 0:
                                formula[1][feature] = o_formula[1][feature]
                                visited[1][feature] = 1
                            else:
                                formula[1][feature] = max(formula[1][feature], o_formula[1][feature])
    ...
    def get_formulae_text(self, file):
        if self._conjunction is True:
            if len(self.groups_formula) == 0:
                self._get_formulae()
            # print(self.groups_formula)
            # print(self.visited)
            # print(self._group_weights)
            for index, formula in enumerate(self.groups_formula):
                text = ''
                for i in range(self._n_features):
                    if self.visited[index][0][i] == 1:
                        text += '(feature_' + str(i) + ' <= ' + '%.1f' % formula[0][i] + ') Λ '
                    if self.visited[index][1][i] == 1:
                        text += '(feature_' + str(i) + ' > ' + '%.1f' % formula[1][i] + ') Λ '
                print('Group' + str(index) + ': |' + str(sum(self.group_weights[index])) + 'samples| ' +
                      str(len(self._groups[index][1])) + 'rules| ' + str(self._groups[index][0]))
                print(text[:-2])
            ...
            for index, group in enumerate(self._groups):
                text = ''
                for r, rule in enumerate(group[1]):
                    text += '\t' + str(self.group_weights[index][r]) + ' samples\t'
                    for i in range(self._n_features):
                        if o_visited[rule][0][i] == 1:
                            text += '(feature_' + str(i) + ' <= ' + '%.1f' % o_formulae[rule][0][i] + ') Λ '
                            # self.scale += 1
                        if o_visited[rule][1][i] == 1:
                            text += '(feature_' + str(i) + ' > ' + '%.1f' % o_formulae[rule][1][i] + ') Λ '
                            # self.scale += 1
                    text = text[:-3] + '\n'
                # text = text[:-3]
                print('Group' + str(index) + ': |' + str(sum(self.group_weights[index])) + 'samples| ' +
    ...
    def classify_a_sample(self, x):
        ...
        if self._conjunction is True:
            for index, formula in enumerate(self.groups_formula):
                sat = True
                for feature in range(self._n_features):
                    if self.visited[index][0][feature] == 1 and x[feature] > formula[0][feature]:
                        sat = False
                        break
                    if self.visited[index][1][feature] == 1 and x[feature] <= formula[1][feature]:
                        sat = False
                        break
                if sat is True:
            ...
            for index, group in enumerate(self._groups):
                ...
                for r, rule in enumerate(group[1]):
                    sat = True
                    for feature in range(self._n_features):
                        if o_visited[rule][0][feature] == 1 and x[feature] > o_formulae[rule][0][feature]:
                            sat = False
                            break
                        if o_visited[rule][1][feature] == 1 and x[feature] <= o_formulae[rule][1][feature]:
                            sat = False
                            break
                    if sat is True:
