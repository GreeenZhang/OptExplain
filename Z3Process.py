...
class Z3Process(object):
    def __init__(self, extractor=None, k=-1):
        # k是size_filter裁剪的比例(k<=0时取全部rule，0<k<1时取k比例的rule，k>=1时取k个rule)
        self._extractor = extractor
        self._groups = []       # [G1,G2...Gk]; Gi:[(签名),[满足签名的rules下标],[经过maxsat运算的rules下标]]
        ...
        self.group_weights = []
        self._k = k
        self.groups_after_filter = []      # 经过filter的groups
    ...
    def maxsat(self):
        ...
        for index, group in enumerate(self._groups):
            ...
            for rule_num in group[1]:
                ...
                opt.add(eval(text))
                opt.add_soft(eval(rule_name), self._values[rule_num].sum())       # 此处maxsat权重目前使用了叶子的value
            opt.check()
            m = opt.model()
            tmp = []
            for decl in m:
                if isinstance(m[decl], z3.z3.BoolRef) and m[decl] == True:      # '=='不能改
            ...
            value_sum1 = 0          # maxsat前的sample数
            value_sum2 = 0          # maxsat后的sample数
    ...
    def filter(self, _k):
        ...
        for i in range(len(self._groups)):
            _group = []
            _group.append(self._groups[i][0])
